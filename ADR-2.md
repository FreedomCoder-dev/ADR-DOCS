...
##todo перенести контекст в обоснование 
...
- ASM
    ...
    - Можно запустить на наших ресурсах
    - Эффективное использование ресурсов благодаря полному контролю над ними
    - В будущем позволит использовать устаревшие компьютеры и смартфоны в качестве узлов кластера для распределения нагрузки
    ...
    - Многопоточка - за пределами мечтаний
    - Гарантированно ***не*** попадём в сроки (даже при увеличении времени на MVP в 5 раз)
    - Сложность разработки 100/100 попугаев
    - Сложность поддержки 100/100 удавов
    - Риск 100/100 наломанных дров
    - В нашей команде есть один специалист по этому языку
- C
    - Очень старомодный
    - Архитектор одобряет
    ...
    - Делит второе место (среди представленных вариантов) по производительности с Rust - идеально для наших ресурсов и расчётной нагрузки
    ...
    - В будущем может позволить использовать устаревшие компьютеры и смартфоны в качестве узлов кластера для распределения нагрузки
    - Есть шанс уложиться в сроки при правильном планировании и опыте команды
    - Сложность разработки 70/100 попугаев
    - Сложность поддержки 80/100 удавов
    - Риск 70/100 наломанных дров
    - В нашей команде есть специалист по этому языку
- C++
	- Очень старомодный
	- Архитектор не одобряет
	- Быстро работает при прямых руках (такие имеются) - идеально для наших ресурсов и расчётной нагрузки
    ...
    - Сложность разработки 70/100 попугаев
    - Сложность поддержки 70/100 удавов
    - Риск 30/100 наломанных дров
    - В нашей команде есть три специалиста по этому языку
- Golang
	- Модный
	- Архитектор одобряет
	...
	- Крутая многопоточка
	- Обширный опыт работы с языком в команде
    - Сложность разработки 70/100 попугаев
    - Сложность поддержки 70/100 удавов
    - Риск 30/100 наломанных дров
...
- Swift
	...
	- Требуются модные компьютеры от фруктовой компании для комфортной разработки (имеется)
    - Пайплайны CD могут быть сложными или нестабильными из-за короткого времени поддержки ОС не на основе Darwin
    - Сложность разработки 2/100 попугаев
    - Сложность поддержки 5/100 удавов
    - Риск 5/100 наломанных дров
    - В нашей команде есть специалист по этому языку
...
- JavaScript (TypeScript)
    ...
    - Сложность разработки 4/100 попугаев
    - Сложность поддержки 5/100 удавов
    - Риск 1/100 наломанных дров
    - В нашей команде есть 2 специалиста по этому языку
...
- Kotlin
	- Модненький
	- Архитектор одобряет
	- Сделано в россии :russia: (отечественное качество :muscle:)
	...
	- JVM безопасен и почти никогда не может жестко упасть при должных механизмах autorecovery - балбес-разработчик может расслабиться
    ...
    - Легко настроить пайплайны CD
    - Сложность разработки 20/100 попугаев
    - Сложность поддержки 30/100 удавов
    - Риск 7/100 наломанных дров
    - В нашей команде есть специалист по этому языку
...
- C#
    ...
    - Медленный
    - В нашей команде нет специалистоф по этому языку
    
- Python
    - Модный
    - Архитектор одобряет
    - Прекрасен в своём нынешнем состоянии
	- Многопоточка сомнительная
    - Бывает медленным даже при прямых руках (такие имеются)
    - Экосистема поражает воображение размерами
    - Легко настроить пайплайны CD
    - Сложность разработки 5/100 попугаев
    - Сложность поддержки 5/100 удавов
    - Риск 1/100 наломанных дров
    - В нашей команде есть специалист по этому языку
